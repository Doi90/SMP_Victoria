...
settings <- read.xlsx(input.file, sheet = "settings", rowNames = TRUE)
nnodes = 20#as.numeric(settings["MCnodes","Value"])

threats.info <- read.xlsx(input.file, sheet = "threats", rowNames = TRUE)
threats <- rownames(threats.info)

action.info <- read.xlsx(input.file, sheet = "actions", rowNames = TRUE)
all.actions <- rownames(action.info)
actions <- all.actions#[-grep("Reveg",all.actions)]

species.info <- read.xlsx(input.file, sheet = "species", rowNames = TRUE)
...
for(actn in names(which(mincells > 1))){
  ...
  costs[[actn]] <- focal(costs[[actn]], w = matrix(1, nbv, nbv), fun = mean, na.rm = TRUE, pad = TRUE) * clip
...
for(lyr in names(costs)[-1]){
  
  costs[[lyr]] <- costs[[lyr]] * 5.0625 / 4 #Matts surfaces were cost per 4 ha pixel
  
} 

costs[["Clearing"]] <- costs[["Clearing"]] * 5.0625

writeRaster(costs, file = file.path(output.folder, "SMPCosts.tif"), format = "GTiff", overwrite = TRUE)
...
minprop.fill <- ifelse(is.na(features$minprop), mean(features$minprop, na.rm = TRUE), features$minprop)
maxprop.fill <- ifelse(is.na(features$maxprop), mean(features$maxprop, na.rm = TRUE), features$maxprop)
...
for(a in 2:length(actions)){
  ...
  totbens <- addLayer(totbens, (clip - 1))
...
writeRaster(totbens, file = file.path(output.folder, "totalbenefits.tif"), format = "GTiff", overwrite = TRUE)
writeRaster(BCratio.all, file = file.path(output.folder, "BCratios.tif"), format = "GTiff", overwrite = TRUE)
...
actions.scen <- actions[actions %in% c("Harvesting", "Phytoph", "Fuelred") == FALSE]
...
date <- Sys.Date()
writeRaster(bestact, file = paste0(output.folder, "/SMPbestaction", date, ".tif"), format = "GTiff", overwrite = TRUE)

##END STAGE 1 PRIORITIZATION

#copy best action cost tif to Zwd folder

file.copy(bestactions.scen$cost.raster, file.path(Zwd, "/inputs/SMP_costbest.tif"), overwrite = TRUE)
