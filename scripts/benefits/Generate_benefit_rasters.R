...
for(k in taxon_id){  
  ...
  ## Simulate benefit draws from a triangle distribution based on expert opinion
  ...
  for(i in seq_len(nrow(responses))){
    ...
    sim_benefits[i, ] <- rtriangle(1000,
                                   a = responses$minBen[i],
                                   b = responses$maxBen[i],
                                   c = responses$meanBen[i])
    
  }
  
  filename <- sprintf("outputs/benefits/simulated_benefits_1000_%s.rds",
                      group)
  
  saveRDS(sim_benefits,
          filename)
  ...
  for(sim in seq_len(1000)){
    
    ## Model training data
    
    mdat <- data.frame(y = sim_benefits[ , sim],
              ...
              if(length(usehdm) > 0){
                ...
                for(actn in mapacts){
                  ...
                  if(actn != "Do Nothing"){
                    ...
                    tifname <- sprintf("Spp%s_delta_%s_sim_%s.tif",
                                       sp,
                                       shortname,
                                       sim) 
                  }
